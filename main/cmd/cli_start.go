...
func mainStart() {
	if localconf.ChainMakerConfig.Plugin.RecorderFile.ConfigServerPort > 0 { // 这里初始化 recorder 包; 如果注释掉 chainmaker.yml 中的 plugin 配置项，则不会对 recorder 包进行初始化。
		port := localconf.ChainMakerConfig.Plugin.RecorderFile.ConfigServerPort
		err := recorderfile.Start(port)
		if err != nil {
			log.Errorf("recorderfile.start(%d) err: %s", port, err.Error())
		} else {
