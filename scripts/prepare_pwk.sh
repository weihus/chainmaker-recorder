...
BC_YML_TRUST_ROOT_LINE=$(awk '/trust roots list start/{print NR}' ${CONFIG_TPL_PATH}/chainconfig/bc_4_10.tpl)
BC_YML_TRUST_ROOT_LINE_END=$(awk '/trust roots list end/{print NR}' ${CONFIG_TPL_PATH}/chainconfig/bc_4_10.tpl)
...
" node$i/chainmaker.yml
        ...
        for ((j = 1; j < $CHAIN_CNT + 1; j = j + 1)); do
            ...
            if  [ $NODE_CNT -eq 1 ]; then
                ...
                cp $CONFIG_TPL_PATH/chainconfig/bc_4_10.tpl node$i/chainconfig/bc$j.yml
                xsed "s%{consensus_type}%$CONSENSUS_TYPE%g" node$i/chainconfig/bc$j.yml
