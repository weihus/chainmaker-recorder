...
import (
	...
	pb "chainmaker.org/chainmaker/libp2p-pubsub/pb"
...
func NewGossipSub(ctx context.Context, h host.Host, logger api.Logger, opts ...Option) (*PubSub, error) {
	...
	// Customize part start
	tagTracer := newTagTracer(h.ConnManager())
	if tagTracer.decayer != nil {
		rt.tagTracer = tagTracer
...
// UpdatePubSubConfig
func (gs *GossipSubRouter) UpdatePubSubConfig(param string, value string) {
	switch param {
	case "Connectors":
		v, _ := strconv.Atoi(value)
		if v > GossipSubConnectors {
			temp := GossipSubConnectors
			GossipSubConnectors = v
			for i := temp; i < v; i++ {
				go gs.connector(i)
			}
		} else {
		...
		gs.p.log.Infof("[pubsub] update Connectors to gossipSub success, value: %d", GossipSubConnectors)
	case "HeartbeatInterval":
		v, _ := strconv.Atoi(value)
		GossipSubHeartbeatInterval = time.Second * time.Duration(v)
		go gs.heartbeatTimer()
		gs.p.log.Infof("[pubsub] update HeartbeatInterval to gossipSub success, value: %d", GossipSubHeartbeatInterval)
	case "GossipRetransmission":
		v, _ := strconv.Atoi(value)
		GossipSubGossipRetransmission = v
		gs.p.log.Infof("[pubsub] update GossipRetransmission to gossipSub success, value: %d", GossipSubGossipRetransmission)
	case "OpportunisticGraftPeers":
		v, _ := strconv.Atoi(value)
		GossipSubOpportunisticGraftPeers = v
		gs.p.log.Infof("[pubsub] update OpportunisticGraftPeers to gossipSub success, value: %d", GossipSubOpportunisticGraftPeers)
	case "Dout":
		v, _ := strconv.Atoi(value)
		gs.Dout = v
		gs.p.log.Infof("[pubsub] update Dout to gossipSub success, value: %d", gs.Dout)
	case "Dlazy":
		v, _ := strconv.Atoi(value)
		gs.Dlazy = v
		gs.p.log.Infof("[pubsub] update Dlazy to gossipSub success, value: %d", gs.Dlazy)
	case "GossipFactor":
		v, err := strconv.ParseFloat(value, 64)
		floatvar := float64(v) / 100.0
		if err != nil {
			gs.p.log.Error(err)
		}
		GossipSubGossipFactor = floatvar
		gs.p.log.Infof("[pubsub] update GossipFactor to gossipSub success, value: %f", GossipSubGossipFactor)
	case "OpportunisticGraftTicks":
		v, _ := strconv.Atoi(value)
		uintvar := uint64(v)
		gs.opportunisticGraftTicks = uintvar
		gs.p.log.Infof("[pubsub] update OpportunisticGraftTicks to gossipSub success, value: %d", gs.opportunisticGraftTicks)
	case "FloodPublish":
		v, _ := strconv.Atoi(value)
		if v > 0 {
			gs.floodPublish = true
		} else {
			gs.floodPublish = false
		}
		gs.p.log.Infof("[pubsub] update FloodPublish to gossipSub success, value: %d", gs.floodPublish)
	default:
		return
	}
}
...
func WithPeerScore(params *PeerScoreParams, thresholds *PeerScoreThresholds) Option {
	return func(ps *PubSub) error {
		...
		if !ok {
			return fmt.Errorf("libp2ppubsub router is not gossipsub")
		}
...
func (gs *GossipSubRouter) Attach(p *PubSub) {
	...
	gs.pid = protocol.ID("/meshsub/1.1.0/" + p.pubsubUid)
...
func (gs *GossipSubRouter) AddPeer(p peer.ID, proto protocol.ID) {
	gs.p.log.Debugf("PEERUP: Add new peer %s using %s", p, proto)
...
loop:
	for _, c := range conns {
		if c.Stat().Direction == network.DirOutbound {
			// only count the connection if it has a libp2ppubsub stream
...
func (gs *GossipSubRouter) RemovePeer(p peer.ID) {
	gs.p.log.Debugf("PEERDOWN: Remove disconnected peer %s", p)
...
func (gs *GossipSubRouter) HandleRPC(rpc *RPC) {
	...
	gs.p.log.Infof("[pubsub] GossipSubGossipRetransmission: %d", GossipSubGossipRetransmission)
...
func (gs *GossipSubRouter) handleIHave(p peer.ID, ctl *pb.ControlMessage) []*pb.ControlIWant {
	...
	if score < gs.gossipThreshold {
		gs.p.log.Debugf("IHAVE: ignoring peer %s with score below threshold [score = %f]", p, score)
		return nil
	}
	...
	if gs.peerhave[p] > GossipSubMaxIHaveMessages {
		gs.p.log.Debugf("IHAVE: peer %s has advertised too many times (%d) within this heartbeat interval; ignoring", p, gs.peerhave[p])
		return nil
	}

	if gs.iasked[p] >= GossipSubMaxIHaveLength {
		gs.p.log.Debugf("IHAVE: peer %s has already advertised too many messages (%d); ignoring", p, gs.iasked[p])
		return nil
	}
	...
	gs.p.log.Debugf("IHAVE: Asking for %d out of %d messages from %s", iask, len(iwant), p)
...
func (gs *GossipSubRouter) handleIWant(p peer.ID, ctl *pb.ControlMessage) []*pb.Message {
	...
	score := gs.score.Score(p)
	if score < gs.gossipThreshold {
		gs.p.log.Debugf("IWANT: ignoring peer %s with score below threshold [score = %f]", p, score)
		return nil
	}
	...
	for _, iwant := range ctl.GetIwant() {
		for _, mid := range iwant.GetMessageIDs() {
			...
			if count > GossipSubGossipRetransmission {
				gs.p.log.Debugf("IWANT: Peer %s has asked for message %s too many times; ignoring request", p, mid)
				continue
			}
			// Customize part start

			if gs.p.pubsubUid != *msg.PubsubUid {
				gs.p.log.Debugf("IWANT: msg pubsub uid[%s] mismatch [%s]; ignoring request", *msg.PubsubUid, gs.p.pubsubUid)
				continue
			}
	...
	gs.p.log.Debugf("IWANT: Sending %d messages to %s", len(ihave), p)
...
func (gs *GossipSubRouter) handleGraft(p peer.ID, ctl *pb.ControlMessage) []*pb.ControlPrune {
	...
	for _, graft := range ctl.GetGraft() {
		...
		_, direct := gs.direct[p]
		if direct {
			gs.p.log.Warnf("GRAFT: ignoring request from direct peer %s", p)
		...
		if backoff && now.Before(expire) {
			gs.p.log.Debugf("GRAFT: ignoring backed off peer %s", p)
		...
		if score < 0 {
			...
			gs.p.log.Debugf("GRAFT: ignoring peer %s with negative score [score = %f, topic = %s]", p, score, topic)
		...
		gs.p.log.Debugf("GRAFT: add mesh link from %s in %s", p, topic)
...
func (gs *GossipSubRouter) handlePrune(p peer.ID, ctl *pb.ControlMessage) {
	...
	for _, prune := range ctl.GetPrune() {
		...
		gs.p.log.Debugf("PRUNE: Remove mesh link to %s in %s", p, topic)
		...
		if len(px) > 0 {
			...
			if score < gs.acceptPXThreshold {
				gs.p.log.Debugf("PRUNE: ignoring PX from peer %s with insufficient score [score = %f, topic = %s]", p, score, topic)
				continue
			}
...
func (gs *GossipSubRouter) pxConnect(peers []*pb.PeerInfo) {
	...
	for _, pi := range peers {
		...
		if pi.SignedPeerRecord != nil {
			...
			if err != nil {
				gs.p.log.Warnf("error unmarshalling peer record obtained through px: %s", err)
				continue
			}
			rec, ok := r.(*peer.PeerRecord)
			if !ok {
				gs.p.log.Warnf("bogus peer record obtained through px: envelope payload is not PeerRecord")
				continue
			}
			if rec.PeerID != p {
				gs.p.log.Warnf("bogus peer record obtained through px: peer ID %s doesn't match expected peer %s", rec.PeerID, p)
				continue
			}
			spr = envelope
		}
	...
	for _, ci := range toconnect {
		...
		case gs.connect <- ci:
		default:
			gs.p.log.Debugf("ignoring peer connection attempt; too many pending connections")
			break
		}
	}
}

func (gs *GossipSubRouter) connector(index int) {
	gs.p.log.Infof("[pubsub] start connector: %d", index)
	for {
		if index >= GossipSubConnectors {
			gs.p.log.Infof("[pubsub] Connectors: %d", GossipSubConnectors)
			return
		}
		select {
		case ci := <-gs.connect:
			gs.p.log.Infof("[pubsub] start connector: %d, in select", index)
			if gs.p.host.Network().Connectedness(ci.p) == network.Connected {
				continue
			}

			gs.p.log.Debugf("connecting to %s", ci.p)
			cab, ok := peerstore.GetCertifiedAddrBook(gs.p.host.Peerstore())
			if ok && ci.spr != nil {
				_, err := cab.ConsumePeerRecord(ci.spr, peerstore.TempAddrTTL)
				if err != nil {
					gs.p.log.Debugf("error processing peer record: %s", err)
				}
			}
			...
			cancel()
			if err != nil {
				gs.p.log.Debugf("error connecting to %s: %s", ci.p, err)
			}
...
func (gs *GossipSubRouter) Publish(msg *Message) {
	...
	for _, topic := range msg.GetTopicIDs() {
		...
		if !ok {
			// Customize part start
			gs.p.log.Debugf("the topic is not in the topics map, topic: [%s]", topic)
			// Customize part end
			continue
		}
		// gs.p.log.Infof("[pubsub] FloodPublish: %t", gs.floodPublish)
		...
		for p := range gs.direct {
			...
			if inTopic {
				tosend[p] = struct{}{}
				// Customize part start
				gs.p.log.Debugf("the direct peer in the topic, direct peer: [%v]", p)
				// Customize part end
			}
		}
		...
		for p := range gmap {
			tosend[p] = struct{}{}
			// Customize part start
			gs.p.log.Debugf("the gossip sub peer in the topic, gossip sub peer: [%v]", p)
			// Customize part end
		}
	}
	...
	for pid := range tosend {
		...
		if !gs.p.whitelist.Contains(pid) {
			continue
		}
...
func (gs *GossipSubRouter) Join(topic string) {
	...
	for p := range gmap {
		gs.p.log.Debugf("JOIN: Add mesh link to %s in %s", p, topic)
		gs.tracer.Graft(p, topic)
...
func (gs *GossipSubRouter) Leave(topic string) {
	...
	gs.p.log.Debugf("LEAVE %s", topic)
	gs.tracer.Leave(topic)

	delete(gs.mesh, topic)

	for p := range gmap {
		gs.p.log.Debugf("LEAVE: Remove mesh link to %s in %s", p, topic)
		gs.tracer.Prune(p, topic)
...
func (gs *GossipSubRouter) doDropRPC(rpc *RPC, p peer.ID, reason string) {
	gs.p.log.Warnf("dropping message to peer %s: %s", p.Pretty(), reason)
...
func fragmentMessageIds(msgIds []string, limit int) [][]string {
	...
	for i := 0; i < len(msgIds); i++ {
		...
		if size > limit {
			// pathogs.p.logical case where a single message ID exceeds the limit.
...
func (gs *GossipSubRouter) heartbeatTimer() {
	...
	//gs.p.log.Infof("[heartbeat] GossipSubHeartbeatInterval: %d", GossipSubHeartbeatInterval)
	gs.ticker = time.NewTicker(GossipSubHeartbeatInterval)
	...
	for {
		...
		case <-gs.ticker.C:
			...
			case gs.p.eval <- gs.heartbeat:
			case <-gs.p.ctx.Done():
				//gs.p.log.Infof("heartbeatTimer stop 1")
				return
			}
		case <-gs.p.ctx.Done():
			//gs.p.log.Infof("heartbeatTimer stop 2")
			return
		}
	}
}
...
func (gs *GossipSubRouter) heartbeat() {
	//defer gs.p.log.EventBegin(gs.p.ctx, "heartbeat").Done()
	// Customize part start
	// gs.p.log.Infof("[pubsub] GossipSubOpportunisticGraftPeers: %d", GossipSubOpportunisticGraftPeers)
	// gs.p.log.Infof("[pubsub] Dout: %d", gs.Dout)
	defer func() {
		gs.p.log.Debugf("heartbeat done.")
	}()
	// Customize part end
	gs.heartbeatTicks++
	...
	for topic, peers := range gs.mesh {
		...
		graftPeer := func(p peer.ID) {
			gs.p.log.Debugf("HEARTBEAT: Add mesh link to %s in %s", p, topic)
			gs.tracer.Graft(p, topic)
			peers[p] = struct{}{}
			topics := tograft[p]
		...
		for p := range peers {
			if score(p) < 0 {
				gs.p.log.Debugf("HEARTBEAT: Prune peer %s with negative score [score = %f, topic = %s]", p, score(p), topic)
				prunePeer(p)
				noPX[p] = true
			}
		}
		...
		if len(peers) > gs.Dhi {
			...
			for _, p := range plst[gs.D:] {
				gs.p.log.Debugf("HEARTBEAT: Remove mesh link to %s in %s", p, topic)
				prunePeer(p)
			}
		}
		...
		// gs.p.log.Infof("[pubsub] OpportunisticGraftTicks: %d", gs.opportunisticGraftTicks)
		...
		if gs.heartbeatTicks%gs.opportunisticGraftTicks == 0 && len(peers) > 1 {
			...
			if medianScore < gs.opportunisticGraftThreshold {
				...
				for _, p := range plst {
					gs.p.log.Debugf("HEARTBEAT: Opportunistically graft peer %s on topic %s", p, topic)
					graftPeer(p)
				}
			}
		}
...
func (gs *GossipSubRouter) applyIwantPenalties() {
	for p, count := range gs.gossipTracer.GetBrokenPromises() {
		gs.p.log.Infof("peer %s didn't follow up in %d IWANT requests; adding penalty", p, count)
...
func (gs *GossipSubRouter) emitGossip(topic string, exclude map[peer.ID]struct{}) {
	...
	if len(mids) > GossipSubMaxIHaveLength {
		...
		gs.p.log.Debugf("too many messages for gossip; will truncate IHAVE list (%d messages)", len(mids))
	}
	...
	for p := range gs.p.topics[topic] {
		...
		_, direct := gs.direct[p]
		if !inExclude && !direct && /*Customize part start*/ /*(gs.peers[p] == GossipSubID_v10 || gs.peers[p] == GossipSubID_v11) */ gs.peers[p] == gs.pid /*Customize part end*/ && gs.score.Score(p) >= gs.gossipThreshold {
			peers = append(peers, p)
		}
	}
	// gs.p.log.Infof("[pubsub] GossipFactor: %f", GossipSubGossipFactor)
	// gs.p.log.Infof("[pubsub] Dlazy: %d", gs.Dlazy)
	target := gs.Dlazy
		...
		for _, p := range peers {
			...
			if ok {
				...
				if spr != nil {
					...
					if err != nil {
						gs.p.log.Warnf("error marshaling signed peer record for %s: %s", p, err)
					}
				}
			}
...
func (gs *GossipSubRouter) getPeers(topic string, count int, filter func(peer.ID) bool) []peer.ID {
	...
	for p := range tmap {
		if /*Customize part start*/ /*(gs.peers[p] == GossipSubID_v10 || gs.peers[p] == GossipSubID_v11)*/ gs.peers[p] == gs.pid /*Customize part end*/ && filter(p) {
			peers = append(peers, p)
		}
	}
