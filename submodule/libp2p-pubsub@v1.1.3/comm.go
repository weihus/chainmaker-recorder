...
func (p *PubSub) handleNewStream(s network.Stream) {
	...
	for {
		...
		if err != nil {
			if err != io.EOF {
				s.Reset()
				p.log.Infof("error reading rpc from %s: %s", s.Conn().RemotePeer(), err)
			} else {
...
func (p *PubSub) handleNewPeer(ctx context.Context, pid peer.ID, outgoing <-chan *RPC) {
	...
	if err != nil {
		...
		p.log.Debugf("opening new stream to peer: err: [%v], pid: [%v]", err, pid)
		// Customize part end
	...
	// Customize part start
	p.log.Debugf("opening new stream to peer: pid: [%v], protocols: [%v]", pid, p.rt.Protocols())
	// Customize part end
...
func (p *PubSub) handlePeerEOF(ctx context.Context, s network.Stream) {
	...
	for {
		...
		p.log.Warn("unexpected message from ", s.Conn().RemotePeer())
	}
}
...
func (p *PubSub) handleSendingMessages(ctx context.Context, s network.Stream, outgoing <-chan *RPC) {
	...
	for {
		...
		case rpc, ok := <-outgoing:
			...
			if err != nil {
				s.Reset()
				p.log.Infof("writing message to %s: %s", s.Conn().RemotePeer(), err)
				return
			}
		case <-ctx.Done():
			return
