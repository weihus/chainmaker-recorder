...
// UpdatePubSubConfig
func (rs *RandomSubRouter) UpdatePubSubConfig(param string, value string) {
	rs.p.log.Infof("[pubsub] update %s to randomSub failed", param)
}
...
func (rs *RandomSubRouter) Publish(msg *Message) {
	...
	for p := range tosend {
		...
		default:
			rs.p.log.Infof("dropping message to peer %s: queue full", p)
