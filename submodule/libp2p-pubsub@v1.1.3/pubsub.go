...
import (
	...
	logging "chainmaker.org/chainmaker/logger/v2"
...
var pLog *logging.CMLogger
var log api.Logger

func init() {
	pLog = logging.GetLogger("[Net]")
}
...
type PubSub struct {
	...
	leaveWhitelistPeer chan peer.ID
...
type PubSubRouter interface {
	...
	Leave(topic string)
	// UpdatePubSubConfig
	UpdatePubSubConfig(param string, value string)
}
...
func (p *PubSub) UpdatePubSubConfig(param string, value string) {
	p.rt.UpdatePubSubConfig(param, value)
}
...
func NewPubSub(ctx context.Context, h host.Host, rt PubSubRouter, opts ...Option) (*PubSub, error) {
	ps := &PubSub{
		...
		leaveWhitelistPeer: make(chan peer.ID),
...
func (p *PubSub) processLoop(ctx context.Context) {
			...
			go func(c chan *RPC) {
				defer func() {
					if err := recover(); err != nil {
						p.log.Error("pubsub process loop defer func goroutine panic error, ", err)
						p.log.Error(debug.Stack())
					}
				}()
				close(c)
			}(ch)
	...
	for {
		...
		case pid := <-p.newPeers:
			// Customize part start
			p.log.Infof("find a new peer: [%v]", pid)
			// Customize part end

			if _, ok := p.peers[pid]; ok {
				p.log.Warn("already have connection to peer: ", pid)
				continue
			}

			if p.blacklist.Contains(pid) {
				p.log.Warn("ignoring connection from blacklisted peer: ", pid)
				continue
			}
			...
			if !p.whitelist.Contains(pid) {
				p.log.Warn("ignoring connection from not whitelisted peer: ", pid)
				continue
			}
		...
		case s := <-p.newPeerStream:
			...
			_, ok := p.peers[pid]
			if !ok {
				p.log.Warn("new stream for unknown peer: ", pid)
				s.Reset()
				continue
			}

			if p.blacklist.Contains(pid) {
				p.log.Warn("closing stream for blacklisted peer: ", pid)
				//close(ch)
				s.Reset()
				continue
			}
		...
		case pid := <-p.peerDead:
			...
			if p.host.Network().Connectedness(pid) == network.Connected {
				...
				p.log.Warn("peer declared dead but still connected; respawning writer: ", pid)
				messages := make(chan *RPC, p.peerOutboundQueueSize)
				messages <- p.getHelloPacket()
				go p.handleNewPeer(ctx, pid, messages)
				p.peers[pid] = messages
		...
		case pid := <-p.blacklistPeer:
			p.log.Infof("Blacklisting peer %s", pid)
			p.blacklist.Add(pid)
		...
		// Customize part start
		case pid := <-p.leaveWhitelistPeer:
			p.log.Infof("leave whitelist peer %s", pid)
		...
		case <-ctx.Done():
			p.log.Info("libp2ppubsub processloop shutting down")
			return
		}
	}
}
...
func (p *PubSub) announce(topic string, sub bool) {
	...
	for pid, peer := range p.peers {
		...
		default:
			p.log.Infof("Can't send announce message to peer %s: queue full; scheduling retry", pid)
...
func (p *PubSub) notifySubs(msg *Message) {
	for _, topic := range msg.GetTopicIDs() {
		...
		for f := range subs {
			...
			case f.ch <- msg:
			default:
				p.log.Infof("Can't deliver message to subscription for topic %s; subscriber too slow", topic)
			}
		}
	}
}
...
func (p *PubSub) handleIncomingRPC(rpc *RPC) {
	// Customize part start
	p.log.Debugf("receiving msg from: [%v]", rpc.from)
	// Customize part end
	...
	if !p.rt.AcceptFrom(rpc.from) {
		p.log.Infof("received message from router graylisted peer %s. Dropping RPC", rpc.from)
		return
	}

	for _, pmsg := range rpc.GetPublish() {
		if !(p.subscribedToMsg(pmsg) || p.canRelayMsg(pmsg)) {
			p.log.Warn("received message we didn't subscribe to. Dropping.")
			continue
		}
		...
		// Customize part start
		p.log.Debugf("receiving msg: [%v]", pmsg)
		// Customize part end
...
func (p *PubSub) pushMsg(msg *Message) {
	...
	if p.blacklist.Contains(src) {
		p.log.Warnf("dropping message from blacklisted peer %s", src)
	...
	if p.blacklist.Contains(msg.GetFrom()) {
		p.log.Warnf("dropping message from blacklisted source %s", src)
	...
	if p.signID != src {
		if !p.whitelist.Contains(src) {
			p.log.Warn("dropping message not from whitelisted peer: ", src)
	...
	// 		p.log.Warn("dropping message not from whitelisted source: ", src)
	// 		p.tracer.RejectMessage(msg, rejectNotWhitelistedPeer)
	// 		return
	// 	}
	// }
	...
	if p.signPolicy.mustVerify() {
		if p.signPolicy.mustSign() {
			if msg.Signature == nil {
				p.log.Debugf("dropping unsigned message from %s", src)
			...
			if msg.Signature != nil {
				p.log.Debugf("dropping message with unexpected signature from %s", src)
			...
			if p.signID == "" {
				if msg.Seqno != nil || msg.From != nil || msg.Key != nil {
					p.log.Debugf("dropping message with unexpected auth info from %s", src)
	...
	if peer.ID(msg.GetFrom()) == self && src != self {
		p.log.Debugf("dropping message claiming to be from self but forwarded from %s", src)
...
func (p *PubSub) publishMessage(msg *Message) {
	...
	if p.pubsubUid != msg.GetPubsubUid() {
		p.log.Warn("dropping message not belong to this pubsub: ", msg.PubsubUid)
