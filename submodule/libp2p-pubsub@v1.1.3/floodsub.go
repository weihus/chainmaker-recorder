...
// UpdatePubSubConfig
func (fs *FloodSubRouter) UpdatePubSubConfig(param string, value string) {
	fs.p.log.Infof("[pubsub] update %s to floodSub failed", param)
}
...
func (fs *FloodSubRouter) Publish(msg *Message) {
	...
	for pid := range tosend {
		...
		default:
			fs.p.log.Infof("dropping message to peer %s: queue full", pid)
