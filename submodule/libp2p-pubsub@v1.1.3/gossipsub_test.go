...
func getGossipsub(ctx context.Context, h host.Host, opts ...Option) *PubSub {
	ps, err := NewGossipSub(ctx, h, pLog, opts...)
	if err != nil {
		panic(err)
	}
	return ps
...
func TestGossipsubRPCFragmentation(t *testing.T) {
	...
	// have the real libp2ppubsub join the test topic
	_, err := ps.Subscribe("test")
	if err != nil {
		t.Fatal(err)
	}

	// wait for the real libp2ppubsub to connect and try to graft to the faker
	time.Sleep(time.Second)
