...
const (
	...
	defaultCacheThreshold = 1               // The number threshold to add tx to txList from txCache
	defaultFlushTimeOut   = 2 * time.Second // The time threshold to add tx to txList from txCache. unit:s
	defaultFlushTicker    = 2               // The time threshold to add tx to txCache from addTxsCh. unit:s
...
func IsMessageTurbo(chainConf protocol.ChainConf) bool {
	if chainConf != nil {
		config := chainConf.ChainConfig()
		if config != nil && config.Core != nil && config.Core.ConsensusTurboConfig != nil {
			return config.Core.ConsensusTurboConfig.ConsensusMessageTurbo
		}
	}
	return false
}
