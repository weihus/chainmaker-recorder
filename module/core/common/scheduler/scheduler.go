...
func (ts *TxScheduler) executeTx(
	...
	// performance start
	ContractStartTime := time.Now().Format("2006-01-02 15:04:05.000000")
	...
	// 合约执行时间
	// performance finish
	ContractEndTime := time.Now().Format("2006-01-02 15:04:05.000000")
	str := fmt.Sprintf("%s,%s,%s,%s\n", strconv.FormatUint(block.Header.BlockHeight, 10), ContractStartTime, ContractEndTime, strconv.FormatUint(txResult.ContractResult.GasUsed, 10)) //需要写入csv的数据，切片类型
	// ContractExecTime := &recorder.ContractExecTime{
	// 	BlockHeight:        int(block.Header.BlockHeight), //snapshot.block.Header.BlockHeight,
	// 	GasUsed:            int(txResult.ContractResult.GasUsed),
	// 	ContractStartTime:  ContractStartTime,
	// 	ContractFinishTime: ContractFinishTime,
	// }
	// recorder.Record(ContractExecTime, nil)
	_ = recorderfile.Record(str, "contract_execute_efficiency")
	// if err != nil {
	// 	if err == errors.New("close") {
	// 		ts.log.Errorf("[contract_execute_efficiency] switch closed")
	// 	} else {
	// 		ts.log.Errorf("[contract_execute_efficiency] record failed")
	// 	}

	// }
	// ts.log.Infof("[contract_execute_efficiency] record succeed")
