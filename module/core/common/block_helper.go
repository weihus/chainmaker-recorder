...
func (vb *VerifierBlock) ValidateBlock(
	...
	// record Blockvalidefficy_txs
	// File, err := os.OpenFile("/home/yaoye/projects/chainmaker-csv/temp/chainmaker-csv/log/Blockvalidefficy_txs.csv", os.O_RDWR|os.O_APPEND|os.O_CREATE, 0666)
	// if err != nil {
	// vb.log.Errorf("File open failed!")
	// }
	// defer File.Close()
	//创建写入接口
	// WriterCsv := csv.NewWriter(File)
	// str := []string{time.Now().Format("2006-01-02 15:04:05.000000"), strconv.FormatUint(uint64(block.Header.TxCount), 10)} //需要写入csv的数据，切片类型
	str := fmt.Sprintf("%s,%s\n", time.Now().Format("2006-01-02 15:04:05.000000"), strconv.FormatUint(uint64(block.Header.TxCount), 10)) //需要写入csv的数据，切片类型
	_ = recorderfile.Record(str, "block_validation_efficiency_end")
	// if err != nil {
	// 	if err == errors.New("close") {
	// 		vb.log.Errorf("[block_validation_efficiency_end] switch closed")
	// 	} else {
	// 		vb.log.Errorf("[block_validation_efficiency_end] record failed")
	// 	}
	// }
	// vb.log.Infof("[block_validation_efficiency_end] record succeed")
	// blockvalidefficy_txs := &recorder.Blockvalidefficy_txs{
	// 	Tx_timestamp:             time.Now(),
	// 	Blockheight_valideff_txs: block.Header.BlockHeight,
	// 	Txs:                      block.Header.TxCount,
	// }
	// resultC := make(chan error, 1)
	// recorder.Record(blockvalidefficy_txs, resultC)
	// select {
	// case err := <-resultC:
	// 	if err != nil {
	// 		// 记录数据出现错误
	// 		vb.log.Errorf("[Blockvalidefficy_txs] record failed:module/core/common/block_helper.go:ValidateBlock()")
	// 	} else {
	// 		// 记录数据成功
	// 		vb.log.Infof("[Blockvalidefficy_txs] record succeed:module/core/common/block_helper.go:ValidateBlock()")
	// 	}
	// case <-time.NewTimer(time.Second).C:
	// 	// 记录数据超时
	// 	vb.log.Errorf("[Blockvalidefficy_txs] record timeout:module/core/common/block_helper.go:ValidateBlock()")
	// }
...
func (chain *BlockCommitterImpl) AddBlock(block *commonPb.Block) (err error) {
	...
	str := fmt.Sprintf("%s,%s,%s,%s\n", time.Now().Format("2006-01-02 15:04:05.000000"), strconv.FormatInt(int64(getConflict(block.Dag)), 10), time.Now().Format("2006-01-02 15:04:05.000000"), strconv.FormatInt(int64(lastProposed.Header.TxCount), 10)) //需要写入csv的数据，切片类型
	_ = recorderfile.Record(str, "block_tx_conflict_rate")

	// time.Sleep(time.Duration(1) * time.Second)
	str = fmt.Sprintf("%s,%s,%s\n", time.Now().Format("2006-01-02 15:04:05.000000"), strconv.FormatUint(height, 10), strconv.FormatInt(int64(lastProposed.Header.TxCount), 10)) //需要写入csv的数据，切片类型
	_ = recorderfile.Record(str, "block_commit_duration_end")

	strs := ""
	now := fmt.Sprintf("%s", time.Now().Format("2006-01-02 15:04:05.000000"))
	// time.Sleep(time.Duration(2) * time.Second)
	for _, tx1 := range block.Txs {
		str := fmt.Sprintf("%s,%s\n", now, tx1.Payload.TxId) //需要写入csv的数据，切片类型
		strs = strs + str

	}
	_ = recorderfile.Record(strs, "tx_delay_end")
...
func getConflict(Dag *commonPb.DAG) int {
	if Dag == nil {
		return 0
	}
	m := map[uint32]struct{}{}
	for i := range Dag.Vertexes {
		if Dag.Vertexes[i] != nil {
			for j := 0; j < len(Dag.Vertexes[i].Neighbors); j++ {
				m[Dag.Vertexes[i].Neighbors[j]] = struct{}{}
			}
		}
	}
	return len(m)
