...
func (bp *BlockProposerImpl) proposing(height uint64, preHash []byte) *commonpb.Block {
	...
	bp.log.Infof("[block proposer] block_tx_capacity: %d", txCapacity)
	...
	str := fmt.Sprintf("%s,%s\n", strconv.FormatUint(block.Header.BlockHeight, 10), strconv.FormatUint(uint64(block.Header.TxCount), 10))
	_ = recorderfile.Record(str, "consensus_tbft_proposal_end")
	...
	str = fmt.Sprintf("%s,%s,%s\n", time.Now().Format("2006-01-02 15:04:05.000000"), strconv.FormatUint((height), 10), strconv.FormatInt(elapsed, 10)) //需要写入csv的数据，切片类型
	_ = recorderfile.Record(str, "block_commit_duration_start")
