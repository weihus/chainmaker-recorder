...
func (v *BlockVerifierImpl) VerifyBlock(block *commonpb.Block, mode protocol.VerifyMode) (err error) {
	...
	// record Blockvalidefficy_total
	// File, err := os.OpenFile("/home/yaoye/projects/chainmaker-csv/temp/chainmaker-csv/log/Blockvalidefficy_total.csv", os.O_RDWR|os.O_APPEND|os.O_CREATE, 0666)
	// if err != nil {
	// 	v.log.Errorf("File open failed!")
	// }
	// defer File.Close()
	//创建写入接口
	// WriterCsv := csv.NewWriter(File)
	str := fmt.Sprintf("%s,%s\n", time.Now().Format("2006-01-02 15:04:05.000000"), strconv.FormatInt(elapsed, 10)) //需要写入csv的数据，切片类型
	_ = recorderfile.Record(str, "block_validation_efficiency_start")
	// if err != nil {
	// 	if err == errors.New("close") {
	// 		v.log.Errorf("[block_validation_efficiency_start] switch closed")
	// 	} else {
	// 		v.log.Errorf("[block_validation_efficiency_start] record filed")
	// 	}
	// }
	// v.log.Infof("[block_validation_efficiency_start] record succeed")
	// blockvalidefficy_total := &recorder.Blockvalidefficy_total{
	// 	Tx_timestamp:         time.Now(),
	// 	Block_valideff_total: newBlock.Header.BlockHeight,
	// 	Total:                elapsed,
	// }
	// resultC := make(chan error, 1)
	// recorder.Record(blockvalidefficy_total, resultC)
	// select {
	// case err := <-resultC:
	// 	if err != nil {
	// 		// 记录数据出现错误
	// 		v.log.Errorf("[Blockvalidefficy_total] record failed:module/core/syncmode/verifier/block_verifier_impl.go:VerifyBlock()")
	// 	} else {
	// 		// 记录数据成功
	// 		v.log.Infof("[Blockvalidefficy_total] record succeed:module/core/syncmode/verifier/block_verifier_impl.go:VerifyBlock()")
	// 	}
	// case <-time.NewTimer(time.Second).C:
	// 	// 记录数据超时
	// 	v.log.Errorf("[Blockvalidefficy_total] record timeout:module/core/syncmode/verifier/block_verifier_impl.go:VerifyBlock()")
	// }
